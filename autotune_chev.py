...
def grid_search_multi(inFile, simu):
    ...
    DAMPING_FACTOR_1 = list(np.arange(0.1, 0.8, 0.1).round(decimals=1))
    SWEEP_1 = list(range(1, 3))
    DEGREE_4 = list(range(1, 6))
    RATIO_EIVAL_4 = list(range(20, 40))
    EIVAL_MAXIT_4 = list(range(5, 100))
    ...
    param_grid_4 = {'chebyshev: degree': DEGREE_4,
                    'chebyshev: ratio eigenvalue': RATIO_EIVAL_4,
                    'chebyshev: eigenvalue max iterations': EIVAL_MAXIT_4
                   }
...
def get_time_gridsearch(filenames, case, simu, iter_time_dict, iter_totaltime_dict, iter_bool_dict):
    '''
    ...
    for each_file in filenames:
        with open(each_file) as f:
            ...
            if simu == 0: 
                ...
                iter_totaltime_dict[output_num] = list()
                iter_bool_dict[output_num] = list() 
            iter_time_dict[output_num].append(time) # update the list of time
            iter_totaltime_dict[output_num].append(totaltime)
            iter_bool_dict[output_num].append(bool_passed)
...
def random_search_multi(inFile, n_iter, seed):
    ...
    DEGREE_4 = list(range(1, 6))
    RATIO_EIVAL_4 = list(range(10, 50))
    EIVAL_MAXIT_4 = list(range(5, 100))
...
if __name__ == "__main__":
    ...
    if algo == "grid-multi":
        ...
        for i in range(num_simu):
            ...
            iter_time_dict, iter_totaltime_dict, iter_bool_dict = get_time_gridsearch(out_filename, casename, i, iter_time_dict, iter_totaltime_dict, iter_bool_dict)
        ...
        iter_totaltime_dict = {k: statistics.median(time_list) for k, time_list in iter_totaltime_dict.items()}
        # round final digits to 4
        iter_time_dict = {k: round(iter_time_dict[k], 4) for k in iter_time_dict}
        iter_totaltime_dict = {k: round(iter_totaltime_dict[k], 4) for k in iter_totaltime_dict}
        iter_bool_dict = {k: any(boo) for k, boo in iter_bool_dict.items()} 
